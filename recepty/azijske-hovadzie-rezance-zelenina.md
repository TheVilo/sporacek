# Ázijské hovädzie rezance so zeleninou

**slug:** azijske-hovadzie-rezance-zelenina
**porcie:** 2
**čas prípravy:** 30 minút
**foto_url:** fotky/azijske-hovadzie-rezance-zelenina.jpg

---

## Foto prompt
A contemporary, bright lifestyle food photo in a 9:16 format, capturing an elevated home cooking moment. Asian-style stir-fried beef strips with rice noodles, bell peppers, carrot and onion in a glossy soy-ginger sauce, topped with sesame seeds, in a modern large matte bowl, centered on a smooth oak table. Next to it, adding depth to the editorial scene, is a smaller minimalist bowl filled with extra soy sauce. Chopsticks rest casually on the bowl, with a casually draped linen cloth nearby. Soft, airy natural window light. No text, no logo, no watermark in the image.

---

## Suroviny (pre 2 osoby)

| Surovina | Množstvo |
|---|---|
| Hovädzie mäso | 300 g (na rezance) |
| Ryžové rezance | 200 g |
| Paprika | 200 g (farebná zmes) |
| Mrkva | 100 g |
| Cibuľa | 1 ks |
| Cesnak | 2 strúčiky |
| Zázvor | kúsok (~2 cm) |
| Sójová omáčka | 3 PL |
| Med | 1 PL |
| Sezamový olej | 1 PL |
| Sezamové semienka | 1 ČL |

---

## Nutričné hodnoty (na porciu, odhad)

| | |
|---|---|
| Energia | ~480 kcal |
| Bielkoviny | 32 g |
| Sacharidy | 50 g |
| Tuky | 16 g |

---

## Postup

1. Ryžové rezance priprav podľa návodu na obale (zvyčajne namočiť v horúcej vode 5–8 minút), scedi.
2. Hovädzie mäso nakrájaj na tenké prúžky, ideálne naprieč vláknami — mäso zostane jemnejšie.
3. Papriku, mrkvu a cibuľu nakrájaj na tenké prúžky/plátky.
4. V miske zmiešaj sójovú omáčku, med a nastrúhaný zázvor na omáčku.
5. Panvicu/wok rozpáľ na vysokom ohni s trochou sezamového oleja. Hovädzie opekaj 1–2 minúty, kým získa farbu, vyber z panvice.
6. Na tej istej panvici opeč cibuľu, mrkvu a cesnak 1 minútu, pridaj papriku, opekaj ďalšie 2 minúty, nech zelenina zostane chrumkavá.
7. Vráť mäso späť, prilej omáčku, pridaj rezance a rýchlo premiešaj, kým sa všetko prehreje a rezance sa obalia omáčkou.
8. Podávaj posypané sezamovými semienkami.

---

## Tagy
`#večera` `#hovädzie` `#sýte` `#jednohrnec`
