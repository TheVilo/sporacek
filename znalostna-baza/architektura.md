# šporáček — architektúra (referencia)

> Detailná referencia dátového modelu, stránok a pravidiel. **Nenačítava sa automaticky** — `CLAUDE.md` sem odkazuje a prečítaš si ju len keď riešiš daný podsystém (build databázy, ceny, social, stránky, formát receptu). Takto `CLAUDE.md` ostáva krátky a nemíňa kontext v každej session.
...
2. `znalostna-baza/aliasy.json` — kurátorované **aliasy** (ako sa tá istá surovina volá v letákoch, napr. `kuracie-prsia` ← „Kuracie prsné rezne", „Kuracie rezne") + **základná jednotka** (g/ml/ks). Toto je tá jediná vec, ktorú treba udržiavať ručne — a je to zámerne tak (rovnako ako `suroviny.md`, párovanie je jediné, čo môže systém rozbiť). Aliasy sa porovnávajú normalizovane (bez diakritiky) ako podreťazec; najdlhší alias vyhráva. **Nepoužívaj priveľmi všeobecné aliasy** (napr. samotné „kuracie") — chytali by nesúvisiace položky. Per surovina tu žije aj `vyluc` (zoznam podreťazcov, pri ktorých sa párovanie **zakáže** — napr. „mandlova" pri múke, inak by mandľová múka za 16 €/kg kazila ceny), `bezna_cena` + `bezne_balenie` (typická cena mimo akcie — fallback pre suroviny, čo nebývajú v letáku: soľ, korenie, múka… user ich väčšinou má doma, ale recept s nimi musí počítať) a `sezona` (mesiace sezóny ovocia/zeleniny).
3. `ceny/*.json` + `recepty/*.md` — cenníky a recepty (bez zmeny, len sa z nich číta).

→ `scripts/build_databaza.py` to spojí do **`docs/data/databaza.json`** (schema_verzia 2): koreň `{ meta, suroviny[], recepty[] }`. Každá surovina má `{ id, nazov, kategoria, jednotka, trvanlivost, gramy_za_ks, spotreba_dni, alergeny[], statistiky, ceny[] }`, kde každá cena nesie `{ obchod, nazov_v_letaku, mnozstvo, balenie_qty, balenie_jednotka, zlavnena_cena, povodna_cena, zlava, jednotkova_cena, platnost_od, platnost_do, podmienka }` — teda **cena + platnosť od-do + podmienka** (napr. „s Lidl Plus"). `statistiky` = bežná/min jednotková cena z celej histórie (pre „zlacnelo o X %" a social fakty). Každý recept má suroviny napárované na `id` **s normalizovaným množstvom** (`qty`, `jednotka`, `mnozstvo_g` — nech appka vie škálovať porcie a zlučovať nákupy bez parsovania textov), odvodené `alergeny[]` + `vegetarianske`/`veganske` (zo surovín, nie ručne), `ceny_za_porciu[]` per obchod (`cena_za_porciu`, `usetris_za_porciu`, `spolahlive`, `nakup[]`) a súhrn `najlacnejsie`. `spolahlive` = ocenená aspoň polovica surovín **a zároveň ≥ 80 % známej hmotnosti receptu** — hmotnostná podmienka bráni tomu, aby lacné základy s bežnou cenou „pretlačili" recept, ktorému chýba cena hlavnej suroviny (mäsa). Recept `.md` **naďalej nikdy neobsahuje cenu** — cena vzniká len tu, pri builde.

**Ceny receptov len z najnovšieho letáku.** `suroviny[].ceny[]` drží históriu (v JSON-e posledných ~12 týždňov; plná história ostáva v `ceny/*.json`), ale `ceny_za_porciu` sa počíta **výhradne z najnovšieho letáku každého obchodu** (`meta.aktualny_letak_od`) — inak by sa po pridaní ďalšieho týždňa miešali staré zľavy s novými. Keď surovina v aktuálnom letáku nie je, použije sa jej `bezna_cena` (v nákupe označená `zdroj_ceny: "bezna"`, v cene receptu počítadlo `odhadom`) — vďaka tomu má **každý recept úplnú cenu**. Presné definície kuchynských mier (PL=15 g, ČL=5 g, štipka=0,5 g, „podľa chuti"=2 g…) sú konštanty v `build_databaza.py` a sú pod regresnými testami `tests/test_build.py` (`python3 -m pytest tests/ -q`; spúšťa ich aj Action pred buildom). Kompletný popis schémy pre programátora appky: `docs/data/SCHEMA.md`.
