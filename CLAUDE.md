...
5. **`docs/stories.html` — vizuálne Stories.** Poskladá hotové **1080×1920** obrázky do Instagram/Facebook Stories z toho istého `tydne/<týždeň>/data.json` (sekcia `social.stories`) + fotky receptu + `cena` (a `bezna_cena` pre úsporu). Ku každému dňu 2 slidy: **hook** (stmavená fotka + pútavá veta + logo) a **recept** (zelené pozadie, fotka s oblými rohmi, pieskový názov, terakota cenový pill „X,XX / PORCIA" so značkou, a úspora „bežne … → ušetríš … %"), plus **1× nákupný zoznam** za celý týždeň (receipt-style: položky po skupinách s cenami, dole cena jedál + úspora). Renderuje na canvas, dá stiahnuť PNG (jeden alebo všetky). Prepínač týždňov/obchodov ako `tyzden.html`.
